...
class SearchService:
    ...
    def aggregate_manga(self, points: list[Any], top_k: int = 10) -> list[dict[str, Any]]:
        ...
        for point in points:
            ...
            if manga_id is None:
                continue
            grouped[int(manga_id)].append((float(point.score), payload))

        ranked: list[dict[str, Any]] = []
        for manga_id, items in grouped.items():
        ...
        ranked.sort(key=lambda row: (-row["score"], -row["hits"], row["manga_id"]))
        return ranked[:top_k]

    def aggregate_packs_for_manga(self, points: list[Any], manga_id: int, top_k: int = 30) -> list[dict[str, Any]]:
        grouped: dict[int, list[tuple[float, dict[str, Any]]]] = defaultdict(list)
        for point in points:
            payload = point.payload or {}
            if int(payload.get("manga_id", -1)) != int(manga_id):
                continue
